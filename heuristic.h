...
void ISDist::init(vector<int> &init_solution)
{
    ...
    // ========== 新增：计算平均原始软子句权重 avg_ori_w ==========
    double avg_ori_w = 0.0;
    if (num_sclauses > 0) {
        long long total_soft_weight_temp = 0;
        for (int c = 0; c < num_clauses; c++) {
            if (org_clause_weight[c] != top_clause_weight) {
                total_soft_weight_temp += org_clause_weight[c];
            }
        }
        avg_ori_w = (double)total_soft_weight_temp / num_sclauses;
    } else {
        avg_ori_w = 1.0;
    }
    
    // ========== 新增：计算M和B集合（如果上一轮有可行解） ==========
    vector<int> M_clauses;
    vector<int> B_clauses;
    
    bool can_use_history = (last_run_feasible == 1 && best_soln_feasible == 1);
    
    if (can_use_history) {
        for (int c = 0; c < num_clauses; c++) {
            if (org_clause_weight[c] == top_clause_weight) continue;
            
            bool sat_in_global = false;
            for (int j = 0; j < clause_lit_count[c]; j++) {
                int var = clause_lit[c][j].var_num;
                bool sense = clause_lit[c][j].sense;
                if (best_soln[var] == sense) {
                    sat_in_global = true;
                    break;
                }
            }
            
            bool sat_in_last = false;
            for (int j = 0; j < clause_lit_count[c]; j++) {
                int var = clause_lit[c][j].var_num;
                bool sense = clause_lit[c][j].sense;
                if (last_run_best_soln[var] == sense) {
                    sat_in_last = true;
                    break;
                }
            }
            
            if (!sat_in_global && sat_in_last) {
                M_clauses.push_back(c);
            } else if (sat_in_global && !sat_in_last) {
                B_clauses.push_back(c);
            }
        }
    }
    
    // 计算bonus值
    double bonus_value = 1.0;
    if (!M_clauses.empty()) {
        long long sum_weights = 0;
        for (int c : M_clauses) {
            sum_weights += org_clause_weight[c];
        }
        bonus_value = (1.0 + sum_weights) / (1.0 + M_clauses.size());
    }
    const double loss_value = 0.5;
    // ========== 新增结束 ==========
    ...
    if (1 == problem_weighted) // weighted partial MaxSAT
    {
        if (0 != num_hclauses)
        {
            ...
            {
                // ========== 修改：把 s_avg 定义移到循环外 ==========
                double s_avg = (double)coe_soft_clause_weight;
                
                for (int c = 0; c < num_clauses; c++)  // <-- 循环在这里！
                {
                    already_in_soft_large_weight_stack[c] = 0;
                    clause_selected_count[c] = 0;
                    ...
                    {
                        // ========== 修改：base_weight 计算移到循环内 ==========
                        double base_weight = org_clause_weight[c] * (s_avg / avg_ori_w);
                        
                        if (!can_use_history) {
                            clause_weight[c] = base_weight;
                        } else {
                            if (last_run_opt_unsat_weight <= opt_unsat_weight) {
                                bool is_in_M = false;
                                for (int mc : M_clauses) {
                                    if (mc == c) { is_in_M = true; break; }
                                }
                                if (is_in_M) {
                                    clause_weight[c] = base_weight * bonus_value;
                                } else {
                                    clause_weight[c] = base_weight;
                                }
                            } else {
                                bool is_in_B = false;
                                for (int bc : B_clauses) {
                                    if (bc == c) { is_in_B = true; break; }
                                }
                                if (is_in_B) {
                                    clause_weight[c] = base_weight * loss_value;
                                } else {
                                    clause_weight[c] = base_weight;
                                }
                            }
                ...
                else
                {
                    if (can_use_history) {
                        if (last_run_opt_unsat_weight <= opt_unsat_weight) {
                            ...
                            clause_weight[c] = is_in_M ? base_weight_pms * bonus_value : base_weight_pms;
                        } else {
                            bool is_in_B = false;
                            for (int bc : B_clauses) {
                                if (bc == c) { is_in_B = true; break; }
                            }
                            clause_weight[c] = is_in_B ? base_weight_pms * loss_value : base_weight_pms;
                        }
                    } else {
...
void ISDist::local_search_with_decimation(char *inputfile)
{
    ...
    for (tries = 1; tries < max_tries; ++tries)
    {
        ...
        if (local_soln_feasible == 1) {
            last_run_feasible = 1;
            last_run_opt_unsat_weight = local_opt;
            for (int v = 1; v <= num_vars; ++v) {
                last_run_best_soln[v] = local_opt_soln[v];
            }
        } else {
            last_run_feasible = 0;
        }
